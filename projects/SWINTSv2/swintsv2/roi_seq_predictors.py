...
class SequencePredictor(nn.Module):
    ...
    def forward(
        ...
        seq_decoder_input = self.DTM(seq_decoder_input)
        ...
        if self.training:
            ...
            if use_beam_search:
                ...
                tmp[:len(words)] = torch.tensor(words)
                word = tmp
                words.append(word)
                decoded_scores.append(char_scores)
            else:
                for batch_index in range(seq_decoder_input_reshape.size(1)):
                    ...
                    word = tmp
                    words.append(word)
                    tmp_score = np.zeros((self.MAX_LENGTH))
                    tmp_score[:len(char_scores)] = torch.tensor(char_scores)
                    decoded_scores.append(tmp_score)
    ...
    def beam_search(self, encoder_context, decoder_hidden, beam_size=6, max_len=32):
        char_score = np.zeros(self.MAX_LENGTH)
        top_seqs = [[(self.MAX_LENGTH, 1.0, char_score, [decoder_hidden])]]
        # loop
