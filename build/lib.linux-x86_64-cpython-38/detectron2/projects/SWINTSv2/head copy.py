...
class DynamicHead(nn.Module):
    ...
    def extra_rec_feat(self, matcher, mask_encoding, targets, N, bboxes, class_logits, pred_bboxes, mask_logits, proposal_features, features):
        ...
        if targets:
            gt_roi_features = self.box_pooler_rec(features, gt_boxes, None)
            pred_roi_features = self.box_pooler_rec(features, proposal_boxes_pred, None)
            ...
            rec_map = self.box_pooler_rec(features, proposal_boxes_pred, None)
            gt_masks = torch.cat(gt_masks).cuda()
...
class RCNNHead(nn.Module):
    ...
    def forward(self, features, bboxes, pro_features, pooler):
        ...
        roi_features = pooler(features, proposal_boxes, None)
        roi_features = roi_features.view(N * nr_boxes, self.d_model, -1).permute(2, 0, 1)        

        # self_att.
        pro_features = pro_features.view(N, nr_boxes, self.d_model).permute(1, 0, 2)
        # attn_mask = self.generate_square_subsequent_mask(nr_boxes)
