...
class SetCriterion(nn.Module):
    ...
    def loss_rec(self, outputs, targets, indices, num_boxes, mask_encoding):
        ...
        # pos_loss = outputs['pred_rec'][1]
        losses = {}
        losses['loss_rec'] = src_rec
        # losses['loss_rec_pos'] = pos_loss
        return losses

    def loss_rpn(self, outputs, targets, indices, num_boxes, mask_encoding):
        """Classification loss (NLL)
        targets dicts must contain the key "labels" containing a tensor of dim [nb_target_boxes]
        """
        losses = outputs['proposal_losses']
        return losses
