...
from detectron2.modeling import META_ARCH_REGISTRY, build_backbone, detector_postprocess, build_proposal_generator
...
class SWINTS(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # weight_dict["loss_rec_pos"] = rec_weight
        weight_dict["loss_rpn_loc"] = 5
        weight_dict["loss_rpn_cls"] = 2
        losses = ["labels", "boxes", "masks", "rec", "rpn"]
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            proposal_boxes = torch.stack([i.proposal_boxes.tensor for i in proposals])
            outputs_class, outputs_coord, outputs_mask,out_rec,decoded_scores = self.head(features, proposal_boxes, proposal_feats, targets, mask_encoding=self.mask_encoding, matcher=self.matcher)
            output = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1], 'pred_masks': outputs_mask[-1], 'pred_rec': out_rec}
            output['proposal_losses'] = proposal_losses
            if self.deep_supervision:
            ...
            proposal_boxes = torch.stack([i.proposal_boxes.tensor for i in proposals])
            outputs_class, outputs_coord, outputs_mask,out_rec, decoded_scores = self.head(features, proposal_boxes, proposal_feats, mask_encoding=self.mask_encoding)
            ...
            results.pred_rec_scores = decoded_scores
            results = [results]
            processed_results = []
    ...
    def prepare_targets(self, targets):
        ...
        for targets_per_image in targets:
            ...
            target["labels"] = gt_classes.to(self.device)[:300]
            target["boxes"] = gt_boxes.to(self.device)[:300]
            target["boxes_xyxy"] = targets_per_image.gt_boxes.tensor.to(self.device)[:300]
            target["image_size_xyxy"] = image_size_xyxy.to(self.device)[:300]
            image_size_xyxy_tgt = image_size_xyxy.unsqueeze(0).repeat(len(gt_boxes), 1)
            target["image_size_xyxy_tgt"] = image_size_xyxy_tgt.to(self.device)[:300]
            target["area"] = targets_per_image.gt_boxes.area().to(self.device)[:300]

            target["gt_masks"] = targets_per_image.gt_masks.to(self.device)[:300]
            masks = target['gt_masks'][:300].crop_and_resize(targets_per_image.gt_boxes[:300], 28)
            target["gt_masks"] = masks.float()[:300]
            target["rec"] = targets_per_image.rec.to(self.device)[:300]
